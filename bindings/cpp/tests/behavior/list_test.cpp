...
class ListBehaviorTest : public OpenDALTest {
 protected:
  void SetUp() override { OpenDALTest::SetUp(); }
};
...
OPENDAL_TEST_F(ListBehaviorTest, ListEmptyDirectory) {
  ...
  // Directory may or may not be included in listing depending on service
  // behavior
...
OPENDAL_TEST_F(ListBehaviorTest, ListDirectoryWithFiles) {
  ...
  // Should contain 3 files, and possibly the directory itself depending on
  // service behavior
  ...
  for (const auto& expected_path : expected_paths) {
    EXPECT_TRUE(actual_paths.count(expected_path) > 0)
...
OPENDAL_TEST_F(ListBehaviorTest, ListNestedDirectories) {
  ...
  for (const auto& expected_path : sub_expected_paths) {
    EXPECT_TRUE(sub_actual_paths.count(expected_path) > 0)
        << "Missing path: " << expected_path;
  }
...
OPENDAL_TEST_F(ListBehaviorTest, ListRootDirectory) {
  ...
  EXPECT_NO_THROW({ auto entries = op_->List("/"); });
}
