...
class TestConfig {
 public:
  ...
  void initialize() {
    ...
    disable_random_root_ =
        (disable_random && std::string(disable_random) == "true");
...
class TestData {
 public:
  static std::string random_string(size_t length = 100) {
    ...
    // Use only alphanumeric characters and safe symbols for maximum
    // compatibility
...
class OpenDALTest : public ::testing::Test {
 protected:
  std::unique_ptr<opendal::Operator> op_;
  ...
  void SetUp() override {
    ...
    try {
      op_.reset(new opendal::Operator(config.service_name(), config.config()));
      ASSERT_TRUE(op_->Available())
          << "Operator not available for service: " << config.service_name();
  ...
  bool supports_write() const { return capability_.write; }

  bool supports_delete() const { return capability_.delete_feature; }

  bool supports_create_dir() const { return capability_.create_dir; }

  bool supports_list() const { return capability_.list; }

  bool supports_copy() const { return capability_.copy; }

  bool supports_rename() const { return capability_.rename; }

  // Helper methods
  std::string random_path() { return TestData::random_path(); }
  std::string random_dir_path() { return TestData::random_dir_path(); }
  std::string random_string(size_t len = 100) {
    return TestData::random_string(len);
  }
  std::vector<uint8_t> random_bytes(size_t len = 100) {
    return TestData::random_bytes(len);
  }
};
...
class BenchmarkTest : public OpenDALTest {
 protected:
  ...
  void benchmark_operation(const std::string& operation_name,
...
inline void initialize_test_framework() { TestConfig::instance().initialize(); }

}  // namespace opendal::test

// Convenience macros
#define OPENDAL_TEST_F(test_fixture, test_name) TEST_F(test_fixture, test_name)
...
#define OPENDAL_SKIP_IF_UNSUPPORTED_WRITE()                                 \
  do {                                                                      \
    if (!this->supports_write()) {                                          \
      GTEST_SKIP() << "Write operations not supported by service: "         \
                   << opendal::test::TestConfig::instance().service_name(); \
    }                                                                       \
  } while (0)
...
#define OPENDAL_SKIP_IF_UNSUPPORTED_DELETE()                                \
  do {                                                                      \
    if (!this->supports_delete()) {                                         \
      GTEST_SKIP() << "Delete operations not supported by service: "        \
                   << opendal::test::TestConfig::instance().service_name(); \
    }                                                                       \
  } while (0)
...
#define OPENDAL_SKIP_IF_UNSUPPORTED_CREATE_DIR()                               \
  do {                                                                         \
    if (!this->supports_create_dir()) {                                        \
      GTEST_SKIP() << "Create directory operations not supported by service: " \
                   << opendal::test::TestConfig::instance().service_name();    \
    }                                                                          \
  } while (0)
...
#define OPENDAL_SKIP_IF_UNSUPPORTED_LIST()                                  \
  do {                                                                      \
    if (!this->supports_list()) {                                           \
      GTEST_SKIP() << "List operations not supported by service: "          \
                   << opendal::test::TestConfig::instance().service_name(); \
    }                                                                       \
  } while (0)
...
#define OPENDAL_SKIP_IF_UNSUPPORTED_COPY()                                  \
  do {                                                                      \
    if (!this->supports_copy()) {                                           \
      GTEST_SKIP() << "Copy operations not supported by service: "          \
                   << opendal::test::TestConfig::instance().service_name(); \
    }                                                                       \
  } while (0)
...
#define OPENDAL_SKIP_IF_UNSUPPORTED_RENAME()                                \
  do {                                                                      \
    if (!this->supports_rename()) {                                         \
      GTEST_SKIP() << "Rename operations not supported by service: "        \
                   << opendal::test::TestConfig::instance().service_name(); \
    }                                                                       \
  } while (0)
